...
func TestDefineTestElementsWithArrayFields(t *testing.T) {
	...
	tests := []struct {
		...
		// Pointer types.
		{
			name: "In pointer array string",
			args: args{
				fieldType:       common.FieldType{BaseType: "string", ComposedType: "*[N]"},
				fieldValidation: "in=a b c",
			},
			want: TestElements{
				conditions:     []string{`obj.myfield != nil && types.SliceOnlyContains(obj.myfield[:], []string{"a", "b", "c"})`},
				concatOperator: "",
				errorMessage:   "myfield elements must be one of 'a' 'b' 'c'",
			},
		},
		{
			name: "Not in pointer array string",
			args: args{
				fieldType:       common.FieldType{BaseType: "string", ComposedType: "*[N]"},
				fieldValidation: "nin=a b c",
			},
			want: TestElements{
				conditions:     []string{`obj.myfield != nil && types.SliceNotContains(obj.myfield[:], []string{"a", "b", "c"})`},
				concatOperator: "",
				errorMessage:   "myfield elements must not be one of 'a' 'b' 'c'",
			},
		},
		{
			name: "In pointer array int",
			args: args{
				fieldType:       common.FieldType{BaseType: "uint8", ComposedType: "*[N]"},
				fieldValidation: "in=1 2 3",
			},
			want: TestElements{
				conditions:     []string{`obj.myfield != nil && types.SliceOnlyContains(obj.myfield[:], []uint8{1, 2, 3})`},
				concatOperator: "",
				errorMessage:   "myfield elements must be one of '1' '2' '3'",
			},
		},
		{
			name: "Not in pointer array int",
			args: args{
				fieldType:       common.FieldType{BaseType: "uint8", ComposedType: "*[N]"},
				fieldValidation: "nin=1 2 3",
			},
			want: TestElements{
				conditions:     []string{`obj.myfield != nil && types.SliceNotContains(obj.myfield[:], []uint8{1, 2, 3})`},
				concatOperator: "",
				errorMessage:   "myfield elements must not be one of '1' '2' '3'",
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := DefineTestElements("myfield", tt.args.fieldType, validation)
			if (err != nil) != wantErr {
